...
namespace OnlineFormCore.Helpers
{
    ...
    public class EnumHelper
    {
        ...
        public static List<_SelectListItem> ConvertDescription<enumType>() where enumType : struct, IConvertible
        {

            var enumList = (from Enum e in Enum.GetValues(typeof(enumType))
                            select new _SelectListItem
                            {
                                DisplayField = GetEnumDescription(e),
                                BindField = Convert.ToInt32(e).ToString()
                            }).ToList();
